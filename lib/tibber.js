...
/**
 * Extract a 17-char VIN from the Tibber device external id.
 * Format observed in the wild: "vendor:VIN" (e.g. "tesla:5YJSA1E26MF1234567")
 * or sometimes the bare external id. We accept either.
 *
 * Returns the VIN if it's exactly 17 chars (the ISO-3779 spec) — that
 * value is also safe as an ioBroker object-id segment. For anything else
 * (malformed external_id, non-VIN devices) returns null so the caller
 * falls back to the device UUID, which is always object-id-safe.
 */
function vinFromDevice(d) {
  const ext = d.externalId || d.external_id || "";
  if (!ext) return null;
  const sep = ext.indexOf(":");
  const candidate = sep >= 0 ? ext.slice(sep + 1) : ext;
  if (candidate.length === 17 && /^[A-Z0-9]{17}$/i.test(candidate)) {
