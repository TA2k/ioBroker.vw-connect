    ...
    try {
      ...
      for (let index in element) {
        ...
        if (
          ...
          this.adapter.log.info("set " + path + "/" + subKey + " - value: " + subValue);
          await this.adapter.setStateAsync(path + "." + subKey, subValue, true);
          continue;
        }
        await this.parse(path + "." + arrayPath, arrayElement, options);
      }
      this.adapter.log.info("Cleanup " + path + " - timestamp: " + aktTimestamp);
      this.cleanupOtherStatesInChannel(path, aktTimestamp);
    } catch (error) {
